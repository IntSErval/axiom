...
export type GoalType      = 'money' | 'count';
export type MilestoneStatus = 'pending' | 'achieved';

export interface Task     { id: string; user_id: string; title: string; description: string | null; priority: 1|2|3|4; status: TaskStatus; project_id: string | null; goal_id: string | null; due_date: string | null; created_at: string; updated_at: string }
...
export interface Goal     { id: string; user_id: string; title: string; goal_type: GoalType; unit: string | null; target_amount: number; current_amount: number; deadline: string | null; status: GoalStatus; created_at: string; habit_id: string | null; category: string | null; completed_at: string | null }
