...
export default function Home() {
  ...
  return (
    <main
      className={`fixed inset-0 flex h-full w-full items-center justify-center overflow-hidden bg-[#080810] transition-opacity duration-500 ease-out ${
        exiting ? 'opacity-0' : 'opacity-100'
      }`}
    >
      <div className="pointer-events-none absolute left-8 top-8 z-10">
        <span className="font-serif text-xl font-bold italic tracking-tight text-white/90">
          AXIOM
        </span>
      </div>
      ...
      >
        <p className="font-serif text-base italic text-white/60">
          Press and hold the card to enter
        </p>
      </div>
    </main>
  );
...
function SceneLoader() {
  return (
    <div className="flex h-full w-full items-center justify-center bg-[#080810]">
      <div className="h-9 w-9 animate-spin rounded-full border-2 border-white/10 border-t-[#6d6a9e]" />
    </div>
  );
}
